            ...
            <div class="wcmp-store-map-filter">
                ...
                <div class="wcmp-inp-wrap">
                    <select name="radiusSelect" id="radiusSelect">
                        <option value=""><?php _e('Within', 'hanata'); ?></option>
                        <?php if($radius) :
                ...
                <div class="wcmp-inp-wrap">
                    <select name="distanceSelect" id="distanceSelect">
                        ...
                        <option value="M" <?php echo selected( $selected_distance, "M", false ); ?>><?php _e('Miles', 'hanata'); ?></option>
                        <option value="K" <?php echo selected( $selected_distance, "K", false ); ?>><?php _e('Kilometers', 'hanata'); ?></option>
                        <option value="N" <?php echo selected( $selected_distance, "N", false ); ?>><?php _e('Nautical miles', 'hanata'); ?></option>
                    </select>
                </div>
                <?php do_action('wcmp_vendor_list_vendor_sort_map_extra_filters'); ?>
                <input type="submit" name="vendorListFilter" value="<?php _e('Submit', 'hanata'); ?>">
            </div>
        </form>
        <div class="wcmp-store-map-pagination">
            <p class="wcmp-pagination-count wcmp-pull-right">
                ...
                if ( $vendor_total <= $per_page || -1 === $per_page ) {
                        ...
                        printf( _n( 'Viewing the single vendor', 'Viewing all %d vendors', $vendor_total, 'hanata' ), $vendor_total );
                } else {
                        $first = ( $per_page * $current ) - $per_page + 1;
                        $last  = min( $vendor_total, $per_page * $current );
                        /* translators: 1: first result 2: last result 3: total results */
                        printf( _nx( 'Viewing the single vendor', 'Viewing %1$d&ndash;%2$d of %3$d vendors', $vendor_total, 'with first and last result', 'hanata' ), $first, $last, $vendor_total );
                }
                ?>
            </p>
                ...
                <div class="vendor_sort">
                    <select class="select short" id="vendor_sort_type" name="vendor_sort_type">
                        ...
                        $vendor_sort_type = apply_filters('wcmp_vendor_list_vendor_sort_type', array(
                            'registered' => __('By date', 'hanata'),
                            'name' => __('By Alphabetically', 'hanata'),
                            'category' => __('By Category', 'hanata'),
                        ));
                    ...
                    ?>
                    <select name="vendor_sort_category" id="vendor_sort_category" class="select"><?php echo wp_kses_post($options_html); ?></select>
                    <?php do_action('wcmp_vendor_list_vendor_sort_extra_attributes'); ?>
                    <input value="<?php echo __('Sort', 'hanata'); ?>" type="submit">
                </div>
            </form>
            ...
            foreach ($vendors as $vendor_id) {
                ...
                <div class="wcmp-store-list">
                    ...
                    <div class="wcmp-profile-wrap">
                        <div class="wcmp-cover-picture" style="background-image: url('<?php if($banner) echo esc_url($banner); ?>');"></div>
                    ...
                    <div class="wcmp-store-detail-wrap">
                        ...
                        <ul class="wcmp-store-detail-list">
                            <li>
                                ...
                                <a href="<?php echo esc_url($vendor->get_permalink()); ?>" class="store-name"><?php echo wp_kses_post($button_text); ?></a>
                            ...
                            <li>
                                ...
                                <p><?php echo wp_kses_post($vendor->get_formatted_address()); ?></p>
                            </li>
                            <?php endif; ?>
                        </ul>
