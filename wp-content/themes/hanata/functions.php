...
function hanata_custom_css() {
	$hanata_settings = hanata_global_settings();
	if (!is_admin()) {
		wp_enqueue_style( 'hanata-style-template', get_template_directory_uri().'/css/template.css'); 
		ob_start(); 
		...
		wp_add_inline_style( 'hanata-style-template', implode($custom_css) );  
	}
}

add_action('wp_enqueue_scripts', 'hanata_custom_css' );

function hanata_custom_js() {
	if (!is_admin()) {
		wp_enqueue_script( 'hanata-script', get_template_directory_uri() . '/js/functions.js', array( 'jquery'), null, true );
		ob_start(); 
		...
		wp_add_inline_script( 'hanata-script', $custom_js );	
	}
}

add_action('wp_enqueue_scripts', 'hanata_custom_js' );
