<?php
     ...
     * @return      void
     */
    class hanata_mega_menu_walker extends Walker_Nav_Menu {
        ...
        function start_el( &$output, $item, $depth = 0, $args = array(), $current_object_id = 0 ) {
            ...
            $output .= $indent . '<li ' . wp_kses_post($value.$class_names) . '>';
			...
			if ( ! empty( $item->megatitle ) ) {				
				...
				if ( ! empty( $item->imagemenu ) ) {
					$item_output .= '<span class="menu-img"><img src="'.esc_url($item->imagemenu).'" alt="'.esc_attr__('Menu Image','hanata').'"/></span>';
				}
				if( !empty( $item->newbadge ) ){
					$item_output .= '<span class="new-badge">'.esc_html__('New','hanata').'</span>';
				}
				if( !empty( $item->salebadge ) ){
					$item_output .= '<span class="sale-badge">'.esc_html__('Hot','hanata').'</span>';
				}				
				$item_output .= $args->link_before . $prepend . apply_filters( 'the_title', $item->title, $item->ID ) . $append;
				$item_output .= $description . $args->link_after;
				$item_output .= '</span>';

				if ( ! empty( $item->imagecontent ) ) {
					$item_output .= '<span class="mega-menu-image"><a href="' . esc_url( $item->url ) . '"><img src="'.esc_url($item->imagecontent).'" alt="'.esc_attr__('Menu Image','hanata').'"/></a></span>';
				}					
    ...
    class Walker_Nav_Menu_Edit_Custom extends Walker_Nav_Menu {
        ...
        function start_el( &$output, $item, $depth = 0, $args = array(), $current_object_id = 0 ) {
            ...
            if ( ! empty( $item->_invalid ) ) {
                ...
                $title = sprintf( __( '%s (Invalid)', 'hanata' ), $item->title );
            } elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
                $classes[] = 'pending';
                $title = sprintf( __( '%s (Pending)', 'hanata' ), $item->title );
            }
	                ...
	                <span class="item-controls">
	                    ...
	                    <span class="item-order hide-if-js">
                            ...
                            ?>" class="item-move-up"><abbr
                                    title="<?php esc_attr__( 'Move up', 'hanata' ); ?>">
                                    &#8593;</abbr></a>
	                        |
	                        <a href="<?php
                                echo esc_url(wp_nonce_url(
                            ...
                            ?>" class="item-move-down"><abbr
                                    title="<?php esc_attr__( 'Move down', 'hanata' ); ?>">&#8595;</abbr></a>
	                    </span>
	                    <a class="item-edit" id="edit-<?php echo esc_attr($item_id); ?>"
                           title="<?php esc_attr__( 'Edit Menu Item', 'hanata' ); ?>" href="<?php
                            echo ( isset( $_GET['edit-menu-item'] ) && $item_id == $_GET['edit-menu-item'] ) ? admin_url( 'nav-menus.php' ) : add_query_arg( 'edit-menu-item', $item_id, remove_query_arg( $removed_args, admin_url( 'nav-menus.php#menu-item-settings-' . $item_id ) ) );
                        ?>"><?php esc_html_e( 'Edit Menu Item', 'hanata' ); ?></a>
	                </span>
            </dt>
        </dl>
        ...
        <div class="menu-item-settings" id="menu-item-settings-<?php echo esc_attr($item_id); ?>">
            ...
            <p class="description description-thin">
                <label for="edit-menu-item-title-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e( 'Navigation Label', 'hanata' ); ?><br/>
            ...
            <p class="description description-thin">
                <label for="edit-menu-item-attr-title-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e( 'Title Attribute', 'hanata' ); ?><br/>
            ...
            <p class="description">
                <label for="edit-menu-item-target-<?php echo esc_attr($item_id); ?>">
                    ...
                    <?php esc_html_e( 'Open link in a new window/tab', 'hanata' ); ?>
                </label>
            </p>

            <p class="description description-thin">
                <label for="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e( 'CSS Classes (optional)', 'hanata' ); ?><br/>
            ...
            <p class="description description-thin">
                <label for="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e( 'Link Relationship (XFN)', 'hanata' ); ?><br/>
            ...
            <p class="description description-wide">
                <label for="edit-menu-item-description-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e( 'Description', 'hanata' ); ?><br/>
                    ...
                    <span
                        class="description"><?php esc_html_e( 'The description will be displayed in the menu if the current theme supports it.', 'hanata' ); ?></span>
                </label>
            </p>
            <?php 
            ...
            <div class="menu-item-actions description-wide submitbox">
                <?php if ( 'custom' != $item->type && $original_title !== false ) : ?>
                    <p class="link-to-original">
                        <?php printf( __( 'Original: %s', 'hanata' ), '<a href="' . esc_attr( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
                    </p>
                <?php endif; ?>
                <a class="item-delete submitdelete deletion" id="delete-<?php echo esc_attr($item_id); ?>" href="<?php
					...
					<?php esc_html_e( 'Remove', 'hanata' ); ?></a> <span class="meta-sep"> | </span> <a
                    class="item-cancel submitcancel" id="cancel-<?php echo esc_attr($item_id); ?>"
                    href="<?php echo esc_url( add_query_arg( array(
                                'edit-menu-item' => $item_id,
                                'cancel'         => time()
                            ), remove_query_arg( $removed_args, admin_url( 'nav-menus.php' ) ) ) );
                    ?>#menu-item-settings-<?php echo esc_attr($item_id); ?>"><?php esc_html_e( 'Cancel', 'hanata' ); ?></a>
            </div>
    ...
    class hanata_alt_menu_walker extends Walker_Nav_Menu {
        ...
        function start_el( &$output, $item, $depth = 0, $args = array(), $current_object_id = 0 ) {
            ...
            $class_names = join( ' ', apply_filters( 'nav_menu_css_class', array_filter( $classes ), $item ) );
            $class_names = ' class="menu-item-' . esc_attr($item->ID) . ' ' . esc_attr( $class_names ) . ' ' . esc_attr($loggedin) . ' ' . esc_attr($loggedout) . ' ' . esc_attr($hashtmlcontent) . '"';
